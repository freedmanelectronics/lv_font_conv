...
// Rotate pixel matrix clockwise by degrees (90, 180, or 270)
function rotate_pixels(pixels, width, height, degrees) {
  if (degrees === 0 || pixels.length === 0) return pixels;

  let result = [];

  switch (degrees) {
    case 90:
      // Clockwise 90°: new[y][x] = old[height - 1 - x][y]
      for (let y = 0; y < width; y++) {
        result[y] = [];
        for (let x = 0; x < height; x++) {
          result[y][x] = pixels[height - 1 - x][y];
        }
      }
      break;
    ...
    case 270:
      // Clockwise 270° (or CCW 90°): new[y][x] = old[x][width - 1 - y]
      for (let y = 0; y < width; y++) {
        result[y] = [];
        for (let x = 0; x < height; x++) {
...
module.exports = async function collect_font_data(args) {
  ...
  for (let dst_code of all_dst_charcodes) {
    ...
    let pixels = ft_result.pixels;
    const { width: g_width, height: g_height } = ft_result;

    // if (ft_result.freetype.glyph_index === 48) { // '0'
    // console.log('=== PRE-ROTATION DEBUG ===');
    // console.log('advance_x:', ft_result.advance_x);
    // console.log('x (bearingX):', ft_result.x);
    // console.log('y (bearingY):', ft_result.y);
    // console.log('width:', ft_result.width);
    // console.log('height:', ft_result.height);
    // console.log('=========================');
    // }

    if (args.rotate > 0) {
      pixels = rotate_pixels(pixels, g_width, g_height, args.rotate);

      // Update bbox based on rotation
      let new_x = ft_result.x;
      let new_y = ft_result.y;
      let new_width = g_width;
      let new_height = g_height;

      if (args.rotate === 90 || args.rotate === 270) {
        // Swap width and height for 90 and 270 degree rotations
        new_width = g_height;
        new_height = g_width;

        if (args.rotate === 90) {
          new_x = ft_result.y;       // Original y becomes x after 90 rotation
          new_y = -ft_result.x - g_width;  // Original x becomes negative y, adjusted by width
        } else {  // 270 rotation
          new_x = ft_result.y - ft_result.height;  // bearingY - height
          new_y = -ft_result.x;                     // -bearingX
        }
      } else if (args.rotate === 180) {
        new_x = -ft_result.x - g_width;  // Flip both coordinates and adjust by width/height
        new_y = -ft_result.y - g_height;
      }
